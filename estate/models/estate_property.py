...
class EstatePropertyTag(models.Model):
    ...
    _sql_constraints = [
        ("check_name", "UNIQUE(name)", "The tag name should be unique."),
    ]
            ...
            if fields.Float.is_zero(self.selling_price, precision_rounding=0.01):
                ...
                if (
                    ...
                    raise exceptions.ValidationError(
                        "The selling price should not be lower than 90 percent of the expected price"
                    )
    ...
    def check_state_before_delete(self):
        for record in self:
            if record.state not in ["new", "canceled"]:
                raise exceptions.ValidationError(
                    'Only properties with the state "New" or "Canceled" can be deleted.'
                )
